...
stack margin: 0, :top => 20, :left => 100 do	 
			...
			para strong("App:  "), ("#{$app_name}")
		    para strong("Suite:  "), ("#{$suite_name}")
			para strong("Marketplace  "), ("#{$mp_name}")
			para strong("Environment  "), ("#{$env_name}")
									   end			
							end
							end
# :items >> an array							
iap_suite = [%w(#{mtu}, #{init}, #{tut_step_complete}, #{hc_sink}, #{sc_sink}, #{onresume}, #{facebook_id}, #{google_id}, #{player_id}, #{package_name}, #{data})]									   
def test_checks    
    mtu = "mtu"
